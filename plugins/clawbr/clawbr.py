...
class ClawbrPlugin(AlleyBotPlugin, ClawbrAPIMixin, ClawbrContentMixin, ClawbrEngagementMixin, ClawbrCommandsMixin, ClawbrDeepIntegrationMixin, ClawbrWalletMixin):
    ...
    def initialize(self, api, core):
        ...
        # Start real-time debate runner (async polling + model router + SyMod scorer)
        try:
            from .clawbr_runner import ClawbrRunner
            self._debate_runner = ClawbrRunner(self)
            self._debate_runner.start()
            print("🎭 ClawbrRunner started — real-time debate polling active")
        except Exception as exc:
            print(f"⚠️ ClawbrRunner failed to start: {exc}")
            self._debate_runner = None
    ...
    def clawbr_auto_debate_command(self, *args) -> str:
        """Create debate with auto-generated opening statement (defensive wrapper)."""
        return ClawbrCommandsMixin.clawbr_auto_debate_command(self, *args)
    ...
    def get_commands(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'clawbr_auto_debate': self.clawbr_auto_debate_command,
    ...
    def clawbr_force_reply_command(self, debate_slug: str = None) -> str:
        ...
        try:
            ...
            # Get debate details
            debate_result = self.get_debate(debate_slug)
            ...
            debate_status = debate_data.get('status', '').lower()
            
            # Check if debate is active
            if debate_status not in ['active', 'open', 'in_progress', 'proposed']:
                return f"❌ **Debate Not Active:** Status is '{debate_data.get('status', 'Unknown')}' - cannot reply"
            
            # If debate is proposed, we need to start it first
            if debate_status == 'proposed':
                print(f"🚀 Starting proposed debate...")
                start_result = self._make_request('POST', f'/debates/{debate_slug}/start')
                if not start_result.get('success', True):
                    return f"❌ **Failed to start debate:** {start_result.get('error', 'Unknown error')}"
                print(f"✅ Debate started successfully")
            
            # Debug: print debate details
            print(f"🔍 Debate Debug:")
            print(f"  Status: {debate_data.get('status')}")
            print(f"  Current Round: {debate_data.get('current_round')}")
            print(f"  Posts: {len(debate_data.get('posts', []))}")
            print(f"  Our Turn: {debate_data.get('isOurTurn', 'unknown')}")
            
            current_round = debate_data.get('current_round', 1)
            
            # Check if this is our first turn (opening argument) or a rebuttal
            posts = debate_data.get('posts', [])
            our_agent_id = self._get_clawbr_agent_id() if hasattr(self, '_get_clawbr_agent_id') else None
            our_posts = [p for p in posts if p.get('authorId') == our_agent_id or p.get('author', {}).get('id') == our_agent_id]
            
            print(f"🔍 Our posts count: {len(our_posts)}")
            
            if len(our_posts) == 0:
                # This is our opening argument - generate opening statement
                topic = debate_data.get('topic', 'the topic')
                prompt = f"""Write an opening argument for a debate about: {topic}

Your opening argument should:
1. State your position clearly
2. Provide 2-3 strong supporting points
3. Be under 750 characters
...
Style: {getattr(self, 'clawbr_debate_style', 'analytical')}"""
                ...
                try:
                    if grok_ai.enabled:
                        argument = grok_ai.chat(prompt, max_tokens=250)
                except:
                    pass
                
                if not argument and deepseek_ai.enabled:
                    try:
                        argument = deepseek_ai.chat(prompt, max_tokens=250)
                    except:
                        pass
                
                if not argument:
                    return f"❌ **Failed to generate opening argument** for debate `{debate_slug}`"
                    
                print(f"📝 Generated opening argument: {argument[:100]}...")
                
            else:
                # This is a rebuttal
                rebuttal = self.generate_debate_rebuttal(debate_slug, "")
                if not rebuttal:
                    return f"❌ **Failed to generate rebuttal** for debate `{debate_slug}`"
                argument = rebuttal
                print(f"📝 Generated rebuttal: {argument[:100]}...")
            
            # Submit the argument
            reply_result = self.submit_debate_argument(debate_slug, argument)
