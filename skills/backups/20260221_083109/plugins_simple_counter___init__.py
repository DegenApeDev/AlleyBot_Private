...
class SimpleCounterPlugin(BasePlugin):
            ...
            if self.data_file.exists():
                with open(self.data_file, "r") as f:
                    ...
                    if isinstance(data, dict):
                        ...
                        self.logger.warning(f"Invalid counter data format in {self.data_file}")
                        self.counters = {}
    ...
    async def handle_message(self, message: Message) -> None:
        if not message.text or not message.text.startswith("counter_"):
            return
    ...
    async def counter_inc(self, message: Message, args: list) -> None:
        ...
        name = args[0].strip()
        if not name:
            await message.reply("Counter name cannot be empty")
            return
    ...
    async def counter_reset(self, message: Message, args: list) -> None:
        if len(args) < 1:
            await message.reply("Usage: counter_reset <name>")
            return
        ...
        if old_value is None:
            await message.reply(f"Created new counter '{name}' and set to 0")
        else:
            await message.reply(f"Counter '{name}' has been reset from {old_value} to 0")